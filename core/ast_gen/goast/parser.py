...
class GoParser(Parser):
    ...
    def func_type(self, p):
        return FuncType(p.field_list0, p.field_list1,p.lineno,p.index)
    ...
    def call_expr(self, p):
        ...
        return CallExpr(p[0], p.args,p.lineno, p.index, ellipsis=ellipsis)
    ...
    def selector_expr(self, p):
        return SelectorExpr(p[0], p[2], p.lineno, p.index)
